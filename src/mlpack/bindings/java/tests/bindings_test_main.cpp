...
// Program Name.
BINDING_NAME("Java binding test");

// Short description.
BINDING_SHORT_DESC(
    "A simple program to test Java binding functionality.");

// Long description.
BINDING_LONG_DESC(
    "A simple program to test Java binding functionality.  You can build "
...
static void mlpackMain()
{
  ...
  if (IO::HasParam("umatrix_order_in"))
  {
    IO::GetParam<arma::Mat<size_t>>("umatrix_order_out") =
        std::move(IO::GetParam<arma::Mat<size_t>>("umatrix_order_in"));
  }

  if (IO::HasParam("row_view_in"))
  {
    IO::GetParam<arma::rowvec>("row_view_out") =
        std::move(IO::GetParam<arma::rowvec>("row_view_in"));
  }

  if (IO::HasParam("col_view_in"))
  {
    IO::GetParam<arma::colvec>("col_view_out") =
        std::move(IO::GetParam<arma::colvec>("col_view_in"));
  }
